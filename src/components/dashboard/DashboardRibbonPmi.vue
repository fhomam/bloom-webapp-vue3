<template>
  <div class="col-span-1 min-[480px]:col-span-1 row-start-3 min-[480px]:row-start-2 md:row-start-1 flex flex-col xl:flex-row justify-end items-end xl:items-center gap-1 xl:gap-5 mt-1 min-[480px]:mt-0 w-full">
    ...
    <div ref="pmiContainerRef" class="flex flex-col items-center min-[480px]:items-end justify-center min-[480px]:pt-1 order-1 xl:order-3 relative w-full min-[480px]:w-auto">
      
      <div class="hidden min-[480px]:flex items-center justify-end gap-1 mb-0.5 w-full">
        <span class="text-[8px] lg:text-[9px] font-bold text-slate-400 uppercase tracking-wider">Quarterly</span>
        <button @click.stop="(e) => toggleMainDetails(e)" class="text-slate-300 hover:text-slate-500 transition-colors cursor-pointer">
      ...
      <div class="flex items-center justify-center min-[480px]:justify-end gap-1.5 lg:gap-2 w-full">
        ...
        <div class="hidden min-[480px]:flex items-end gap-[2px] lg:gap-[3px] h-3.5 lg:h-5 mr-1">
            ...
            <div v-if="uiState.activeBarIdx === idx" 
              ...
              <div class="flex flex-col gap-0.5 text-left">
                ...
                <template v-if="q.status === 'valid'">
                  ...
                  <span class="text-[10px] text-slate-500 mt-1 leading-tight">Based on <strong>{{ formatNumber(q.interactionVolume) }}</strong> interactions</span>
                </template>
              ...
              <div class="absolute -bottom-1.5 w-3 h-3 bg-white border-b border-r border-slate-200 transform rotate-45" :style="uiState.popupPointerStyle"></div>
            </div>
          </div>
        </div>

        <div class="flex items-baseline min-[480px]:items-center gap-1.5 min-[480px]:pb-0">
          <span :class="['text-[28px] min-[480px]:text-3xl lg:text-[28px] font-extrabold tracking-tight leading-none', pmiMetrics.scoreClass]">
            {{ pmiMetrics.scoreDisplay }}
          </span>
          <span class="text-[11px] font-bold text-slate-400 uppercase tracking-wider min-[480px]:hidden">
            PMI
          </span>
        </div>
        
        <span v-if="pmiMetrics.delta !== null" :class="['hidden min-[480px]:block text-[11px] lg:text-sm font-bold', pmiMetrics.delta > 0 ? 'text-rose-500' : 'text-emerald-500']">
          {{ pmiMetrics.delta > 0 ? '+' : '' }}{{ pmiMetrics.delta }}%
        </span>
      </div>

      <div class="flex min-[480px]:hidden items-center justify-center gap-1 mt-0.5 text-[9px] font-bold text-slate-500 uppercase tracking-wider">
        Quarterly
        <button @click.stop="toggleMainDetails" class="text-slate-300 hover:text-slate-500 transition-colors">
      ...
      <span class="hidden min-[480px]:block text-[8px] lg:text-[10px] font-bold text-slate-500 uppercase tracking-wider mt-1 text-right w-full">
        Product Market Impedance
      </span>

      <div 
        v-if="uiState.showMainDetails" 
        class="absolute top-full mt-2 w-64 bg-white border border-slate-200 shadow-xl rounded-xl p-4 z-50 text-left animate-in fade-in zoom-in-95 duration-200"
        :style="uiState.mainPopupStyle"
      >
...
const toggleMainDetails = (event) => {
  uiState.activeBarIdx = null
  if (uiState.showMainDetails) {
    uiState.showMainDetails = false
    return
  }
  uiState.showMainDetails = true
  ...
  if (event) {
    ...
    if (spaceRight < 140) {
      uiState.mainPopupStyle = { right: '0px', left: 'auto', transform: 'none' }
    } else if (spaceLeft < 140) {
      uiState.mainPopupStyle = { left: '0px', right: 'auto', transform: 'none' }
    } else {
      uiState.mainPopupStyle = { left: '50%', transform: 'translateX(-50%)' }
    }
  }
}
